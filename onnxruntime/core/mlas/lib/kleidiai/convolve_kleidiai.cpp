...
static void ConvolveSme(const size_t co, //channels out
    ...
    for (size_t g = 0; g < groups; ++g) {
        ...
        auto lhs_ptrs = GetOrCreateLhsPtrTableSme(ci, ih, iw, d_kh, d_kw, sh, sw, padding, in,&pad_ptr[0]);
        ...
        MlasTrySimpleParallel(ThreadPool, static_cast<ptrdiff_t>(dim[0] * dim[1] * dim[2]), [&](ptrdiff_t tid) {
            ...
            for (size_t m_base = 0; m_base < tile_m_size; m_base += m_chunk) {
                ...
                const size_t chunkSize= std::min(m_chunk, tile_m_size - m_base);

                // Pack chunkSize rows of the LHS matrix into a temporary buffer.
                ...
                if (ArmKleidiAI::UseSME2) {
                        KLEIDIAI_KERNEL_LOG("kai_run_imatmul_clamp_f32_f32p2vlx1_f32p2vlx1b_2vlx2vl_sme2_mopa" << " M=" << chunkSize << " N=" << TileSizeN << " k_chunk_count=" << (d_kh * d_kw) << " k_chunk_length=" << ci);
                        kai_run_imatmul_clamp_f32_f32p2vlx1_f32p2vlx1b_2vlx2vl_sme2_mopa(
                            chunkSize, TileSizeN, d_kh * d_kw, ci, ATile, BTile, CTile, co * sizeof(float),
                            -std::numeric_limits<float>::max(), std::numeric_limits<float>::max()
                        );
                    } else {
                        KLEIDIAI_KERNEL_LOG("kai_run_imatmul_clamp_f32_f32p2vlx1_f32p2vlx1b_2vlx2vl_sme_mopa" << " M=" << chunkSize << " N=" << TileSizeN << " k_chunk_count=" << (d_kh * d_kw) << " k_chunk_length=" << ci);
